function expt = AF_plotsamewellsmeans(expt, varargin)

% Plot the mean signal from samewells in map format
%
...
ylabel = 'Relative Fluorecense Units (RFU)';

% Input data into figure
figure
for i = 1:length(expt.samewells)
    subplot(subplotrows, subplotcols, i);
    titleh = title(expt.samewells(i).label);
    fixfonts('handle', titleh, 'size', 10);
    hold on
    plot(expt.samewells(i).timemean, expt.samewells(i).datamean, ...
        getplottype(i,6,1));
...
set(findall(gcf, 'Type', 'line'), 'LineStyle', '-','LineWidth',1.5);
set(findall(gcf, 'Type', 'axes'), 'Box', 'on','LineWidth',1.5);
fixfonts();
filename = strcat(expt.datadir,'\mean RFU');
