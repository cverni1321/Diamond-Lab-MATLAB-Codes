function c = text2cell(filename, varargin)

% Read a .txt file generated by the FlexStation into a cell array. Empty cells will be returned as an
...
% makenum  - (optional) convert the cells to numbers instead of text, default: 1
...
%%%%%
% Constants (I kept these the same as Mei's code)
%%%%%
...
%%%%%
% Process input arguments (Again, I left these unchanged)
%%%%%
...
while 1
    i = i+1;
    txt = fgetl(fid); % Read in all lines of the .txt file, one line at a time
    if (~ischar(txt))
        fclose(fid);
    ...
    while j < length(txt)
        ...
        elseif strcmp(txt(j), prot) % An unprotected protection character
            if ttidx > 0
                error('Unprotected protection character in cell (%g,%g)', i, thiscol);
            end
            protected = 1;
        elseif protected && strcmp(txt(j), sep) % This is a protected separator, leave it in
            ttidx = ttidx + 1;
            thistext(ttidx) = txt(j);
        elseif strcmp(txt(j), sep) % An unprotected separator
            writenow = 1;
        elseif j == length(txt) % The last character in the line
            if protected
        ...
        % Now we are ready to write the data to an array
        if writenow
            thiscell = thistext(1:ttidx);
            if (i >= row) && (thiscol >= col)
                if makenum % See if the string is numeric before trying to call str2num,
                           % which converts a string matrix to a numeric
                           % array.
