...
% which the mean and standard error of the response to each dose level is
% plotted along with the fitted Hill Equation sigmoid. The EC50 is also
% labeled. Requires nlinfit from statistics toolbox.
%
%   Example
...
[coeffs,~,~] = nlinfit(dose,response,sigmoid,[minResponse maxResponse midResponse 1]);
ec50 = coeffs(3);
hillCoeff = coeffs(4);
...
% Notate the EC50
text(ec50,mean([coeffs(1) coeffs(2)]),[' \leftarrow ' sprintf('IC_{50}=%0.2g nM',ec50)],'FontSize',16);
